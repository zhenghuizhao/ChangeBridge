...
def calc_diversity(data_dir, num_samples=5):
    ...
    for i in tqdm(range(total), total=total, smoothing=0.01):
        ...
        for j in range(num_samples):
            # img = Image.open(os.path.join(os.path.join(data_dir, dir_list[i], f'output_{str(j)}.bmp')))
            img = Image.open(os.path.join(os.path.join(data_dir, str(i), f'output_{str(j)}.bmp')))
            img = img.convert('RGB')
            img = transform(img)
            img = img * 255.
            imgs.append(img)
